...
TEST_CASE("HttpLLMProvider retries a Grok Responses generation failure after output",
          "[integration][http][retry][grok][responses]") {
    httplib::Server server;
    std::atomic<int> attempts{0};
    server.Post("/v1/responses",
        ...
        if (++attempts == 1) {
            res.set_content(
                ...
                "event: error\n"
                "data: {\"type\":\"error\",\"message\":\"Internal error during token generation\"}\n\n",
                "text/event-stream");
            return;
        }
        res.set_content(
