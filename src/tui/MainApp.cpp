...
bool is_letter_codepoint(int codepoint, char letter) {
    const unsigned char lower = static_cast<unsigned char>(std::tolower(static_cast<unsigned char>(letter)));
...
bool is_ctrl_letter_event(const Event& event, char letter) {
...
bool is_ctrl_x_event(const Event& event) {
...
bool is_ctrl_d_event(const Event& event) {
...
public:
    ...
    // Part of the Tab cycle so the user can Tab into history for keyboard scrolling.
    ...
    bool OnEvent(Event event) override {
        ...
        if (Focused()) {
            ...
            if (event == Event::Escape || event == Event::Return) {
                return false;  // let the container handle it / fall through
            }
        }
        return false;
    }
...
RunResult run(RunOptions opts) {
    ...
    if (opts.resume_session_id.has_value()) {
        ...
        if (data_opt.has_value()) {
            ...
            // Prepend a resume banner and reconstruct ui_messages.
            {
                ...
                ui_messages.push_back(make_system_message(std::format(
                    ...
                    "Type a message to continue, or /sessions to manage sessions.\n",
                    data.session_id, ts, data.provider, data.model, data.stats.turn_count)));

                // Replay messages into the UI, including tool call activity.
                // Track the current assistant UI message index so that tool
                // result messages can update the matching tool activity's status.
                ...
                for (const auto& msg : data.messages) {
                    ...
                    } else if (msg.role == "tool" && current_asst_idx >= 0) {
                        // Resolve the matching tool call and mark it done.
